...
if __name__ == '__main__':
    ...
    for ii_fold in range(0, 8):
        ...
        if args.architecture in ['baseline', 'relu_dense',
                                 'no_dense', 'temporal',
            ...
            # organize dataset -------
            test_cv_filename = join(args.path_cv,
                                    '8-fold_cv_random_' + str(ii_fold) + '.fold')
            train_fns = getTrainingFilenames(
                args.path_annotation, test_cv_filename)
            ...
            temp_folder_bock = join(args.path_input, 'temp')
            if not os.path.exists(temp_folder_bock):
                os.makedirs(temp_folder_bock)

            filename_train_validation_set = join(temp_folder_bock, 'feature_bock_' + str(ii_fold) + '.h5')
            filename_labels_train_validation_set = join(temp_folder_bock, 'labels_bock_' + str(ii_fold) + '.pkl')
            filename_sample_weights = join(temp_folder_bock, 'sample_weights_bock_' + str(ii_fold) + '.pkl')
            filename_scaler = join(temp_folder_bock, 'scaler_bock_' + str(ii_fold) + '.pkl')

            saveFeatureLabelSampleweights(
                feature_all, label_all, sample_weights_all, scaler,
            ...
            file_path_model = join(args.path_output, args.architecture + str(ii_fold) + '.h5')
            file_path_log = join(args.path_output, args.architecture + str(ii_fold) + '.csv')

            # architecture -----------------------
            if args.architecture in ['baseline',
                                     'relu_dense',
                                     'no_dense',
                                     'temporal',
                                     '9_layers_cnn',
